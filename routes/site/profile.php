...
$app->group('/profile', function (RouteCollectorProxy $group) {
	...
	$group->group('/orders', function (RouteCollectorProxy $subgroup) {
		...
		$subgroup->get('/{idorder:[0-9]+}', function (Request $request, Response $response, array $args) {
			User::verifyLogin(false);
			$order = new Order();
	...
	$group->group('/change-password', function (RouteCollectorProxy $subgroup) {
		$subgroup->get('', function (Request $request, Response $response) {
			User::verifyLogin(false);
			$page = new Page();
